...
def test_configuration_loader_no_config_directory(mock_missing_config) -> None:
    """
    ...
    mock_missing_config.mkdir(parents=True)


def test_configuration_loader_empty_config(mock_empty_config) -> None:
    """
    ...
    """
    config = Configuration()
    assert mock_empty_config.is_dir()
    assert config.__path__.is_file()


def test_configuration_loader_defaultconfig(mock_default_config) -> None:
    """
    Test initializing oodi configuration loader when the configuration file
    is default yaml configuration file with setings from mocked test data
    """
    config = Configuration()
    assert mock_default_config.is_dir()
    assert config.__path__.is_file()
